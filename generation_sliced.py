...
class setting():
    ...
    #~~~~~~~~~~~~~~ Generating Occupancy measures & Saving Samples ~~~~~~~~~~~~~~~~~~~~
    ...
    def occupancy_generation_fast(self,iteration=0):
        ...
        # self.create_MAF_flow(final_policy_data_num,file_location)

        final_policy_occup,_ = self.MAF_flow(
    ...
    def MAF_flow(self,steps,file_location): #using_finite_episodes
        ...
        xy = np.hstack([x,y]) #shape (N,a+b)
        print('xy: ',np.array(xy).shape) #shape (N,a+b)

        #normalize
        scaler = StandardScaler()
        xy_scaled = scaler.fit_transform(xy) #shape (N,a+b)
        print('xy_scaled: ',np.array(xy_scaled).shape) #shape (N,a+b)
        

        #######################################################
        #prepare data
        xy_tensor = torch.tensor(xy_scaled, dtype=torch.float32)
        dataset = TensorDataset(xy_tensor)
        loader = DataLoader(dataset, batch_size=256, shuffle=True)
        # print(xy_tensor.shape)
    
        #define flow model
        dim = xy_tensor.shape[1]
        print('dim: ', dim)
        num_layers = 5
        hidden_features = 128
        num_flows=5
        ...
        xxx
    #     #save flow model for use later
    #     self.save_flow_model_params(flow)


    # def use_MAF_flow(self,steps,file_location):
    ...
    #     flow.load_state_dict(torch.load("maf_flow.pth"))
    #     flow.eval()

        #evaluate the learned density
        with torch.no_grad():
            log_p = flow.log_prob(xy_tensor)  # log p(x,y)
            p_xy = torch.exp(log_p)           # actual density values

        print(f"\nEstimated mean log p(x,y): {log_p.mean().item():.3f}")
        print(f"Estimated mean p(x,y): {p_xy.mean().item():.3e}")
        print('Here_2')
        xxx

    def save_flow_model_params(self,flow):
        num_model = 0
        file_name = f"flow_{num_model}.pth"
        file_location = f'flow_models/{task}'
        os.makedirs(join(this_dir, file_location), exist_ok=True)
        file_path = abspath(join(this_dir, file_location, file_name))
        
        # save the flow network
        torch.save(flow.state_dict(), file_path)
