...
gamma = 0.99
lr_actor = 1e-3 #3e-4 #ref: mujoco_td3.py
lr_critic = 1e-3 #3e-4 #ref: mujoco_td3.py
exploration_noise = 0.2 #ref: mujoco_td3.py (2018)
...
##Training loop_3
log_path = os.path.join("../runs/{}/td3_{}".format(task,args.n_count))
...
def save_best_fn(policy):
    file_name = "td3_{}.pth".format(args.n_count)
...
def policy_model_saver(policy, step, epoch):
    ...
    if step % M == 0:
        ...
        file_location = f'../td3/{task}/count_{args.n_count}/iter_{args.n_iteration}'
        os.makedirs(join(this_dir, file_location), exist_ok=True)
        file_path = abspath(join(this_dir, file_location, file_name))
