...
parser.add_argument("--encoding_dim", type=int, default=3)
...
class RANDOM_AGENT():
    ...
    def evaluate(self):
        ...
        dataset = pd.DataFrame(stored_states)
        dataset.to_csv(f'{file_location}/dim_{encoding_dim}/data.csv',index=False)
        return stored_states #pd.DataFrame(stored_states) #dataset_size = num_eps*steps_per_episode
...
class Processing():
    ...
    def train(self):
        ...
        optimizer = optima.Adam(self.model.parameters(),lr=lr)
        ...
        for epoch in range(num_epochs): 
            for states in self.loader:
                ...
                latent,reconstructed = self.model(states)
