...
"""class cVAE(nn.Module):
    def __init__(self, x_dim, y_dim, z_dim=8, hidden=[256,256],learn_decoder_var=True):
        ...
        self.encoder_mu = nn.Linear(self.encoder_net.out_dim,
                                    z_dim) #mean_estimation
        self.encoder_logvar = nn.Linear(self.encoder_net.out_dim,
                                        z_dim) #log_variance_estimation

        #decoder p(x|z,y)
        dec_in = z_dim + y_dim
        self.decoder_net = MLP(dec_in,hidden)
        self.decoder_mu = nn.Linear(self.decoder_net.out_dim,
                                    x_dim) #mean_estimation
        if self.learn_decoder_var:
            self.decoder_logvar = nn.Parameter(torch.full((x_dim,),-3.0))
        else:
    ...
    def decode(self,z,y):
        ...
        h = self.decoder_net(zy)
        mu_x = self.decoder_mu(h)

        if self.learn_decoder_var:
            logvar_x = self.decoder_logvar.unsqueeze(0).expand(
                mu_x.shape[0],-1)
        else:
            logvar_x = torch.full_like(mu_x,-3.0)
        return mu_x, logvar_x
    
    def reparameterize(self,mu,logvar):
        logvar = torch.clamp(logvar,-10,10)# -10, 10)
        std = torch.exp(0.5*logvar)
        eta = torch.randn_like(std)
        return mu + eta*std
    
    def forward(self,x,y):
        mu_z, logvar_z = self.encode(x,y) #encode
        z = self.reparameterize(mu_z,logvar_z) #latent
        mu_x, logvar_x = self.decode(z,y) #decode
...
"""
def loss_function(model,x,y,beta=1.0):
    recon_mu, recon_logvar, q_mu, q_logvar, z = model(x,y)

    # reconstruction NLL 
    recon_var = torch.exp(0.5*recon_logvar)
    recon_nll = 0.5*( np.log(2*np.pi) + recon_logvar + 
                     ((x - recon_mu)**2)/recon_var )
    recon_nll = recon_nll.sum(dim=1)

    #KL between q(z|x,y) and prior N(0,I)
    q_logvar = torch.clamp(q_logvar,-10,10) # -10, 10)
    q_std = torch.exp(0.5*q_logvar)

    q_dist = Normal(q_mu,q_std) #approx. posterior: N(u,v)
    p_dist = Normal(torch.zeros_like(q_mu), torch.ones_like(q_std)) #prior: N(0,I)
    kl = kl_divergence(q_dist,p_dist).sum(dim=1)

    #ELBO = - (recon_nll + beta*kl)
    elbo = recon_nll + beta*kl

    # print('recon_nll : ', recon_nll.mean())
    # print('kl : ', kl.mean())
    # print('elbo : ', elbo.mean())
    return elbo.mean() #, recon_nll.mean(), kl.mean()
"""
...
class setting():
    ...
    def verify_policy_converged(self,iteration=0):
        ...
        step = 200 #1200 #len(os.listdir(file_location))-1
        print('step: ', step)
    ...
    def policy_path_generation(self,iteration=0):
        ...
        Args:
            iteration: int, 
        """
        self.iteration = args.n_iteration
        # execution  
        # file_location = f'{algo}/{task.lower()}/samples_{cnt}/iter_{self.iteration}/dim_{encoding_dim}_100k'
        ...
        successive_distance_pair_list = []
        to_final_distance_pair_list = [(initial_policy_data_num,final_policy_data_num)] 
        ...
        for update in range(start_step,len(os.listdir(file_location)),frequency):
            ...
            # successive_distance_pair_list.append((
            #     policy_data_num_first,policy_data_num_second))
            
            # to_final_distance_pair_list.append((
            #     policy_data_num_second,final_policy_data_num))
        ...
        # print('successive_distance_pair_list: \n', len(successive_distance_pair_list))
        # print('to_final_distance_pair_list: \n', len(to_final_distance_pair_list))
        # for i in zip(successive_distance_pair_list,to_final_distance_pair_list):
        #     print(i)

        # print(successive_distance_pair_list[-1])
        # print(to_final_distance_pair_list[-1])
        # xxxx
        return        
    ...
    def policy_path_evaluation(self,iteration=5):
        
        # data_extraction
        self.iteration = 2 #1 #iteration
        num_update = 196 #1951 #
    ...
    def save_images_for_paper(self,iteration=0):
        ...
        self.sampling_factor = 500 #2500
        total_num_updates = 20000 #100000
    ...
    # policy_path_generation_in_policy_space
    def CVAE_evaluation(self,iteration=0):
        """
        Outputs and saves Wasserstein distances across successive 
        policies and between the final policy and 
        successive policies during training
        ...
        print('CVAE_evaluation STARTING')
        ...
        # execution  
        file_location = f'{algo}/{task.lower()}/samples_{cnt}/iter_{self.iteration}'
        policy_data_num = 88 #52

        model1 = self.OTDD_cVAE(policy_data_num,file_location,epochs=200)
        

        #extract states anc actions samples
        states_i, actions_i = self.dataset(policy_data_num,file_location)

        #mini-batch stochastic OT
        num_samples = actions_i.shape[0] #states_i.shape[0]
        size_batch = 2000 #0 #512
        wasserstein = 0

        idx = np.random.choice(num_samples,size_batch,replace=False)
        states_i_batch = states_i[idx]
        actions_i_batch = actions_i[idx]
        # print('len(states_i_batch): ', len(states_i_batch))

        avg_val, avg_Wr = [],[]
        # X = []
        for id, i in enumerate(zip(states_i_batch,actions_i_batch)):
            # print('count_state: ', id)
            # print('i: ', i)

            # print('1: ',i[0])
            X = []
            for idx,j in enumerate(actions_i_batch):
                d_a = LA.norm(i[1] - j)
                if d_a <= 0.1:
                    # print('2: ',i[0])
                    # print('x_n: ', states_i_batch[idx])
                    # X.append(i[0])
                    X.append(states_i_batch[idx])

            Xa = np.asarray(X) #neighbours
            # print('Xa: ', Xa.shape)
            # print('X: ', X)

            Wr = self.single_to_many_Wasserstein(i[0],Xa)
            # print('reference_W: ', Wr)
            avg_Wr.append(Wr)

            #model_samples
            states_given_a1 = self.sample_p_x_given_y(i[1],model1,n_samples=size_batch)
            Z = states_given_a1 #sampled states corresponding to action_i
            # print('Z: ', Z.shape) 

            a = np.ones(Xa.shape[0]) / Xa.shape[0]
            b = np.ones(Z.shape[0]) / Z.shape[0]


            #cost matrix: squared Euclidean distances
            C = ot.dist(Xa,Z,metric='euclidean')**2
            # print('C: ', C)
        ...
        avg_Wr = np.asarray(avg_Wr)
        avg_val = np.asarray(avg_val)
        print('count: ', avg_val.shape[0])
        print('mean: ', avg_val.mean())
        print('std: ', avg_val.std())

        # print('avg_Wr: ', avg_Wr)
        print('mean_Ref: ', avg_Wr.mean())
        print('std_Ref: ', avg_Wr.std())
        ...
        bb = ax.boxplot(d,
                        ...
                        notch='True',
                        tick_labels=labels)
        # bb = plt.boxplot(avg_val,patch_artist=True)
        # patch = bb['boxes']
        # patch.set_facecolor('#ffff00')

        for patch, color in zip(bb['boxes'],colors):
            patch.set_facecolor(color)

        for median in bb['medians']:
        ...
        # for kdx,k in enumerate(states_i_batch):
        #     for ldx,l in enumerate(states_given_a1):
        #         d_s = LA.norm(k - l) #state_in_P(S,A) - state_in_P(S|a)
        #         m[kdx,ldx] = d_s
    ...
    def single_to_many_Wasserstein(self,x,Z): 
        #prep
        x = x.reshape(1, -1) 
        ...
        #weight_for_point_mass (observation)
        a = np.array([1.0])

        #weights for empirical distribution of model samples
        b = np.ones(Z.shape[0]) / Z.shape[0]

        #cost matrix: squared Euclidean distances
        CM = ot.dist(x,Z,metric='euclidean')**2
    ...
    #Optimal_Transport_Dataset_Distance for evaluating cVAE
    def OTDD_cVAE(self,steps_i,file_location,epochs): #using_finite_episodes
        """
        Outputs Wasserstein distance between two policy dataset
        D_i = {(s_k,a_k)}_{k>=1} and D_j = {(s_k,a_k)}_{k>=1}
        ...
        #probability distribution of states given actions
        model_i = self.CVAE(states_i,actions_i,epochs=epochs)
        #states,actions,batch_size=256,epochs=50,lr=1e-3,beta=0.9
        return model_i
