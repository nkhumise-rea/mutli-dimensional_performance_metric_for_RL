...
class setting():
    ...
    def policy_path_evaluation(self,iteration=5):
        ...
        # data_extraction
        self.iteration = 2 #3 #iteration
        num_update =  379 #400 #1951 #
    ...
    def save_images_for_paper(self,iteration=0):
        # data_extraction
        self.iteration = 3 #2 #iteration
        num_update = 400 #[321] #379 #1951 #
        file_name = f'traj_till{num_update}.npy'
        file_location = f'{algo}/{task.lower()}/path_{cnt}/iter_{self.iteration}/dim_{encoding_dim}_100k'
        file_path = abspath(join(this_dir,file_location,file_name)) #file_path: location + name
        data = np.load(file_path, allow_pickle=True)   
        ...
        """
        successive_distance_list = data[0]
        to_final_distance_list = data[1]
        print('successive_distance_list: ', successive_distance_list)
        print('to_final_distance_list: ', to_final_distance_list)

        num_update2 = 391 #379 #1951 #
        file_name2 = f'traj_till{num_update2}.npy'
        file_location2 = f'{algo}/{task.lower()}/path_{cnt}/iter_{self.iteration}/dim_{encoding_dim}_100k'
        file_path2 = abspath(join(this_dir,file_location2,file_name2)) #file_path: location + name
        data2 = np.load(file_path2, allow_pickle=True)   
        print('data: \n', data2)


        successive_distance_list2 = data2[0]
        to_final_distance_list2 = data2[1]
        print('successive_distance_list2: ', successive_distance_list2)
        print('to_final_distance_list2: ', to_final_distance_list2[1:])        
        

        successive_distance_list.extend(successive_distance_list2)
        to_final_distance_list.extend(to_final_distance_list2[1:])
        print('successive_distance_list: ', successive_distance_list)
        print('to_final_distance_list: ', to_final_distance_list)
        ...
        self.sampling_factor = 1000 #900 #2500
        total_num_updates = 40000 #38000 #100000
